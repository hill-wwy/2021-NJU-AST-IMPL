...
class ConvAE:
    ...
    def __init__(self,x,params):  #x==inputs_vae  img_dim
        self.x = x
        a = tf.shape(self.x)[0]
...
#clustering layer
        ...
        z_mean_1 = z_mean + pertation #advisor模型

        z,resample = Lambda(sampling)([z_mean, z_log_var])
        z_1 = z_mean_1+resample
        self.encoder = Model(x, z_mean)
    #vae 合并    x_val_y = ConvAE.vae.predict(x_val)[2]
...
#defense
        ...
        z1 = Dense(latent_dim, activation='relu')(z1)
        z1 = Dense(1, activation='sigmoid')(z1)
        #updates_losses，有.updates属性？
        self.Discriminator = Model(z_in, z1)            #todo
    ...
    #
        ...
        #updates_losses
        self.train_step1 = tf.train.AdamOptimizer().minimize(self.loss_defense,var_list=[self.vae.weights,
    ...
    def train_defense(self, x_train_unlabeled,x_dy,x_dy1,batch_size):
        # create handler for early stopping and learning rate scheduling  外

        losses = self.train_vae_step(
                #这里是两个调整参数怎么来的
                #   之前已经步骤计算得      updates_losses
    ...
    def train_vae_step(self,return_var, updates, x_unlabeled, inputs,x_dy,x_dy1,
        ...
        for batch_num in range(0, batches_per_epoch):  #每次batch
            feed_dict = {K.learning_phase(): 1}  #.K.learning_phase()，返回训练模式/测试模式的flag，
            # 该flag是一个用以传入Keras模型的标记，以决定当前模型执行于训练模式下还是测试模式下.
